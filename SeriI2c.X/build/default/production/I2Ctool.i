...
Byte str2Byte(Byte *b) {
    Byte mode = (Byte)0;
    Byte data = 0;
    while((0 ? isspace(*b) : ((*b == ' ') || ((unsigned)(*b)-'\t' < 5)))) b++;
    if(*b == '0') {
        b++;
        if(*b == 'x' || *b == 'X') {
            mode = (Byte)1;
    ...
    if(mode == (Byte)0) {
        while((0 ? isdigit(*b) : ((unsigned)(*b)-'0') < 10)) {
            data *=10;
            data += (*b - '0');
            b++;
        }
        ...
        while(isxdigit(*b)) {
            data << 4;
            if((0 ? isdigit(*b) : ((unsigned)(*b)-'0') < 10)) data += (*b - '0');
...
Byte str2Hxs(Byte *s, Byte *r) {
    Byte n = 0,d;
    while((0 ? isspace(*s) : ((*s == ' ') || ((unsigned)(*s)-'\t' < 5)))) s++;　
    if(*s == '0' && (*(s+1) == 'x' || *(s+1) == 'X') ) {
        s += 2;
    }
    while(isxdigit(*s)) {
        n++;
        d = ((0 ? isdigit(*s) : ((unsigned)(*s)-'0') < 10)?(*s - '0'):
              (('A'<= *s && *s <='F')?(*s - 'A' + 10):
                  (*s - 'a' + 10)))<<4;
        s++;
        if(isxdigit(*s)) {
...
Byte *skip2blank(Byte *b) {
 if(!(0 ? isspace(*b) : ((*b == ' ') || ((unsigned)(*b)-'\t' < 5))) && *b != '\0') b++;
 return b;
}
